...
fn load_data() -> CResult<Dataset> {
    Ok(candle_datasets::vision::mnist::load()?)
}
...
pub fn criterion_benchmark_std(c: &mut Criterion) {
    ...
    group.bench_function("adadelta", |b| {
        b.iter(|| {
            training::run_training::<Mlp, Adadelta>(m.clone()).expect("Failed to setup training")
        })
    });
    group.bench_function("adagrad", |b| {
        b.iter(|| {
            training::run_training::<Mlp, Adagrad>(m.clone()).expect("Failed to setup training")
        })
    });
    group.bench_function("adam", |b| {
        b.iter(|| training::run_training::<Mlp, Adam>(m.clone()).expect("Failed to setup training"))
    });
    group.bench_function("adamax", |b| {
        b.iter(|| {
            training::run_training::<Mlp, Adamax>(m.clone()).expect("Failed to setup training")
        })
    });
    group.bench_function("esgd", |b| {
        b.iter(|| {
            training::run_training::<Mlp, MomentumEnhancedSGD>(m.clone())
                .expect("Failed to setup training")
        })
    });
    group.bench_function("nadam", |b| {
        b.iter(|| {
            training::run_training::<Mlp, NAdam>(m.clone()).expect("Failed to setup training")
        })
    });
    group.bench_function("radam", |b| {
        b.iter(|| {
            training::run_training::<Mlp, RAdam>(m.clone()).expect("Failed to setup training")
        })
    });
    group.bench_function("rmsprop", |b| {
        b.iter(|| {
            training::run_training::<Mlp, RMSprop>(m.clone()).expect("Failed to setup training")
        })
    });
    // group.bench_function("lbfgs", |b| {
    //     b.iter(|| training::run_lbfgs_training::<Mlp>(m.clone()).expect("Failed to setup training"))
    // });
