...
/* The results of this test have been checked against the following PyTorch code.
    ...
    with torch.no_grad():
        m.weight.zero_()
        m.bias.zero_()
    optimizer = optim.NAdam(m.parameters())
    # optimizer.zero_grad()
...
/* The results of this test have been checked against the following PyTorch code.
    ...
    with torch.no_grad():
        m.weight.zero_()
        m.bias.zero_()
    optimizer = optim.NAdam(m.parameters(), weight_decay = 0.6)
    # optimizer.zero_grad()
    for _step in range(100):
        optimizer.zero_grad()
        ys = m(sample_xs)
...
/* The results of this test have been checked against the following PyTorch code.
    ...
    with torch.no_grad():
        m.weight.zero_()
        m.bias.zero_()
    optimizer = optim.NAdam(m.parameters(), weight_decay = 0.6, decoupled_weight_decay=True)
    # optimizer.zero_grad()
    for _step in range(100):
        optimizer.zero_grad()
        ys = m(sample_xs)
